...
const MyRequests = async () => {
    ...
    const myRequestedDataPromised = await fetch(`${process.env.NEXT_PUBLIC_LOCAL_URI}/public/all-adopted-pets`);
    const allRequestedData = await myRequestedDataPromised.json();
    const myRequestedData = allRequestedData.filter(rq => rq.buyeremail === session?.user?.email ? session.user.email : '')
                                                ...
                                                return (
                                                    <tbody key={d.name} className="divide-y divide-zinc-100 text-sm">
                                                        ...
                                                        >
                                                            ...
                                                            <td className="p-4 whitespace-nowrap">
                                                                <span
                                                                    className={`${d.requeststatus === 'Pending'
